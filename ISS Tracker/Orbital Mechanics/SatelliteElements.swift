...
struct SatelliteElements {
  ...
  var xndt20 = 0.0
  var epoch = 0.0
  func actan(_ sinx: Double, _ cosx: Double) -> Double {
    var ret = 0.0
    if cosx < 0.0 {
      ret = .pi
    } else if cosx == 0.0 {
  ...
  func sgp4(_ tsince: Double) -> (position: Vec3, velocity: Vec3) {
    var isSimple = false
  ...
  func sdp4(_ offsetMinutes: Double) -> (position: Vec3, velocity: Vec3) {
